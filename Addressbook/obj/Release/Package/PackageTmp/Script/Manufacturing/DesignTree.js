...
var DesignTreeView = {
        ...
        DesignTreeView.variables.dx_txtSearch = $("#dx_txtSearch").dxAutocomplete({
            ...
            dataSource: new DevExpress.data.CustomStore({
                load: function (loadOptions) {
                    ...
                    var result;
                    $.ajax({
                        url: getDomain() + DesignTreeView.variables.BindDesignListUrl + "&myfilters=" + JSON.stringify(myfilter),
                        async: false,
                        cache: false,
                        type: 'POST',
                        success: function (data) {
    ...
    SaveSharingDetails: function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                if ($(data).find("RESPONSECODE").text() == 0) {
                    var ShareLink = getDomain() + "/Sharing/DesignShared?ShareId=" + $(data).find("SHAREID").text();
                    var message = "";
                    if (Type == "E-Mail") {
