...
var DevExVariables = {
    ...
    Toaster: function (Type, Message) {
        DevExpress.ui.notify(Message, Type,Type == "success" ? 100 : 2000);
    },
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.indexOf('Error') > -1) {
                    DevExVariables.Toaster("warning", 'Something is Missing');
                }
                else {
                    window.open(getDomain() + result + "?Code=" + Math.random(), '_blank');
                }
            },
            error: OnError
        });
